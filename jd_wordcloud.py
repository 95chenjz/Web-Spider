...
back_color = imread('cz.jpg')  # 解析该图片

wc = WordCloud(background_color='white',  # 背景颜色
               max_words=100000,  # 最大词数
               mask=back_color,  # 以该参数值作图绘制词云，这个参数不为空时，width和height会被忽略
               max_font_size=90,  # 显示字体的最大值
               width=600,
               height=600,
               #stopwords=STOPWORDS.add('Web'),  # 使用内置的屏蔽词，再添加


               font_path="C:/Windows/Fonts/msyhbd.ttc",  # 解决显示口字型乱码问题，可进入C:/Windows/Fonts/目录更换字体
               random_state=4#42,  # 为每个词返回一个PIL颜色
               )
...
jieba.add_word('京东拍卖')
jieba.add_word('Web crawler')



# 打开词源的文本文件
text = open('TTBT2.txt', encoding='utf-8').read()
